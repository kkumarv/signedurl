...
public class V4SignedUrlCallout extends SigningCalloutBase implements Execution {
  ...
  private String getHashedCanonicalRequest(
      final MessageContext msgCtxt, final Map<String, String> serviceAccountInfo, String filepath) throws Exception {
    // CanonicalRequest =
    //   HTTP_VERB + "\n" +
    //   PATH_TO_RESOURCE + "\n" +
    ...
    String resource = getResource(msgCtxt, filepath);
  ...
  private String getStringToSign(
    ...
    //   HASHED_CANONICAL_REQUEST
	  msgCtxt.setVariable(varName("hello_string_to_sign"), "dingding");
    String stringToSign =
        rsaSigningAlgorithm
            + "\n"
            ...
            + "\n"
            + getHashedCanonicalRequest(msgCtxt, serviceAccountInfo, filepath);
...
public ExecutionResult execute(final MessageContext msgCtxt, final ExecutionContext execContext) {
    try {
      ...
      JSONArray FinalSignedUrlList = new JSONArray();
      JSONParser jsonParser = new JSONParser();
      String requestContent = msgCtxt.getVariable("request.content");
      JSONObject jsonObject = (JSONObject) jsonParser.parse(requestContent);
	  JSONArray jsonArray = (JSONArray) jsonObject.get("resources"); 
	  @SuppressWarnings("unchecked")
	  Iterator<String> iterator = jsonArray.iterator();
	  msgCtxt.setVariable(varName("startiterte_now_formatted"), jsonArray);
        while(iterator.hasNext()) {
        	String currentele = iterator.next();
        	  Map<String, String> serviceAccountInfo = getServiceAccountKey(msgCtxt);
              String stringToSign = getStringToSign(msgCtxt, serviceAccountInfo, currentele);  
              ...
              String singleUrl = resolvePropertyValue(V4_SIGNED_URL_SPEC, msgCtxt); 
              FinalSignedUrlList.add(singleUrl);
        }
        msgCtxt.setVariable(varName("signedurl"), FinalSignedUrlList.toString());
